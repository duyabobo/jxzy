...
# 检查配置文件是否存在
if [ ! -f "etc/bsllm.yaml" ]; then
    echo "❌ 配置文件 etc/bsllm.yaml 不存在"
    exit 1
fi

echo "🔧 步骤1: 删除旧的可执行文件..."
# 删除旧的可执行文件
if [ -f "../../bin/bs-llm" ]; then
    rm -f "../../bin/bs-llm"
    echo "🗑️  已删除旧的可执行文件"
fi
...
# 确保logs目录存在
mkdir -p ../../logs
nohup ../../bin/bs-llm -f etc/bsllm.yaml >> ../../logs/access.log 2>&1 &
SERVICE_PID=$!
